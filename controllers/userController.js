...
const sendOtpMail = async (email, otp) => {
  try {
    ...
    const mailOptions = {
      ...
      subject: "For reset Password",
      html:
        "<p> Your Shoemart registration one time password is  " + otp + " </p>",
    };
    transporter.sendMail(mailOptions, function (error, info) {
      if (error) {
        res.render("error");
...
const forgetVerify = async (req, res) => {
  try {
    ...
    if (userData) {
      ...
      res.render("forgetPassword", {
        message: "Email is incorrect , user not found",
      });
    }
  } catch (error) {
    res.render("error");
...
const invoiceDownload = async (req, res) => {
  try {
    ...
    doc
      .fontSize(16)
      .text(`Order Summary - Order ID: ${order.orderId}`, { align: "center" });
    doc.moveDown();
    ...
    order.product.forEach((product, index) => {
      doc
        .fontSize(12)
        .text(`${index + 1}. ${product.name}`, { width: 200, continued: true });
      ...
      doc
        .fontSize(12)
        .text(`${product.quantity}`, { width: 50, align: "right" });
      doc.moveDown();
    ...
    doc
      .fontSize(12)
      .text(`Total Amount with discount: ${order.total}`, { align: "right" });
    doc.moveDown();
    doc
      .fontSize(12)
      .text(
        `Ordered Date: ${order.date.toLocaleDateString("en-US", {
          year: "numeric",
          month: "short",
          day: "numeric",
    ...
    doc
      ...
      .text(
        `Payment Method: ${
          order.payment_method === "1" ? "Cash on Delivery" : "Razor Pay"
        }`
      );
    doc.moveDown();
    ...
    doc
      .fontSize(14)
      .text("Thank you for purchasing with us!", { align: "center" });
