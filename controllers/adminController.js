 ...
 const usersList =async(req,res)=>{
  try{
    ...
    const userData =   await User.find({
     ...
     res.render('userslist',{users:userData, adminMessage: req.session.adminMessage});
  }catch(error){
     console.log(error.message);
    }
 }
...
const updateCategoryList =async(req,res)=>{
  ...
  try{
    ...
    if (req.body.category == "" || /^\s*$/.test(categoryName)){
      ...
      const categoryData=await Category.findByIdAndUpdate({_id:req.body.id},{$set:{category:req.body.category}})
      res.redirect('/admin/category');
    }}else {
      res.render("editCategory",{message:"The Category already exists.",category: categoryData});
    }
  }
  ...
  try {
    if (req.body.category !== "") {
      ...
      if (result.length === 0) {
        ...
        res.render("addnewcategory",{message:"The Category already exists."});
      }
    } else {
 ...
 const productList = async(req,res)=>{
  try{
    ...
    const productData =   await Product.find({isDeleted: false,
     ...
     res.render('product',{product:productData,adminMessage: req.session.productMessage});
  }catch(error){
     console.log(error.message);
    }
 }
...
const updateProduct =  async (req, res) => {
 ...
 const newProductLoad = async(req,res)=>{
  try{
    const categoryData =   await Category.find({});
      res.render('addnewproduct',{category:categoryData, message:req.session.messager})
  }
  catch(error){
...
const deleteProduct = async (req, res) => {
  try {
    const id = req.body.id;
    const response = await Product.findByIdAndUpdate(id, { $set: { isDeleted: true } });
    res.json(response);
  } catch (error) {
    console.log(error.message);
  }
